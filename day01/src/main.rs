fn main() {
    ...
    let distances: Vec<u32> = left_list
        .iter()
        .zip(&right_list)
        .map(|numbers| numbers.0.abs_diff(*numbers.1))
        .collect();
    ...
    let similarity_scores: Vec<i32> = left_list
        .iter()
        .map(|num| num * right_list.iter().filter(|n| *n == num).count() as i32)
        .collect();
